const { sequelize, CommitteeModel} = require("../../models");
...
exports.data_list = async (req, res, next) => {
  ...
  let query_str = "";
  if(search !== ""){
    query_str = " WHERE committee_name like " + '%'+search+'%';
  }

  const query_data = await sequelize.query(`SELECT * FROM committee_list ${query_str} ORDER BY id DESC LIMIT ${offset}, ${limit};`, { type: QueryTypes.SELECT });
  const query_data_count = await sequelize.query(`SELECT COUNT(*) AS num_of_row FROM committee_list ${query_str};`, { type: QueryTypes.SELECT });

  query_data.forEach(function(e) { e.action = CommonFunction.action_menu_edit_del_add_assign_committee(e.id, "committee") });
...
exports.add = [
  ...
  async (req, res, next) => {
    ...
    if(errors.errors.length !== 0){
      ...
      const save_date = await CommitteeModel.create(insert_data).catch(errorHandler);
      req.flash('success', 'Data add successfully!');
      res.redirect('/committee/add');
    }
  }];
...
exports.edit_from = async (req, res, next) => {
  ...
  let result = await CommitteeModel.findOne({ where: {id: id}, order: [ [ 'id', 'DESC' ]] }).catch(errorHandler);
...
exports.edit = [
  ...
  async (req, res, next) => {
    ...
    if(errors.errors.length !== 0){
      ...
      const update_date = await CommitteeModel.update(update_data, { where: { id: req.body.id } }).catch(errorHandler);
...
exports.delete = async (req, res, next) => {
  ...
  const results = await CommitteeModel.destroy({where:{id:req.body.del_id}}).catch(errorHandler);
  return res.status(200).json({
    success: true,
    result: results
  });
