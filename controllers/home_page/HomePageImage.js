...
exports.media_list_add = [async (req, res, next) => {
    ...
    uploads(req, res, async ( err ) => {
        ...
        let media_list = await MediaModel.findAll({ where: {parent_id: id, type: "HomePage"}, order: [ [ 'id', 'ASC' ]] }).catch(errorHandlerProductList);
        ...
        if(media_list.length !== 0){
            ...
            if(isSave){
                for (let j=0;j<files.length;j++){
                    ...
                    const save_date = await MediaModel.create(insert_data).catch(errorHandlerProductList);
                }
